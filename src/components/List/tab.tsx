...
export function TabPanel(props: TabPanelProps) {
  ...
  // Load durable history from main
  const everFound = window.Main.getEverFound() as Record<string, boolean>;

  // Helper: does this item appear as "found" in the UI?
  const isFoundView = (itemName: string) => {
    // in stash (normal or eth) counts as found
    const inStash = !!player[itemName] || !!ethPlayer[itemName];
    // when setting enabled, historical find counts too
    return inStash || (appSettings.persistFoundOnDrop && !!everFound[itemName]);
  };

  // Shows badge when not in stash, but found in history
  const showPrevBadge = (itemName: string) =>
    !player[itemName] &&
    !ethPlayer[itemName] &&
    appSettings.persistFoundOnDrop &&
    !!everFound[itemName];
      ...
      const filterWithOnlyMissing = (object: { [key: string]: any }): { [key: string]: any } => {
        ...
        Object.keys(object).forEach((key: string) => {
          if (typeof object[key] === 'object' && Object.keys(object[key]).length > 0) {
            ...
            // Show only items that are NOT found in the current view (stash OR historical when enabled)
            if (!isFoundView(simplified)) {
              out[key] = object[key];
            }
          }
        });
                              ...
                              >
                                <ListItemButton>
                                  {gameMode !== GameMode.Manual && <>
                                    ...
                                    ) : showPrevBadge(itemName) ? (
                                      <CountLabelContainer title={t('Previously found')}>
                                  ...
                                  <ListItemText
                                    primary={
                                      <span>
                                        {t(itemFullName)}
                                        {showPrevBadge(itemName) && (
                                          <small style={{ opacity: 0.6, marginLeft: 8 }}>(Previously found)</small>
                                        )}
                                      </span>
                                    }
                                  />
                          ...
                          <ListItemText
                            primary={
                              <h3 style={{ display: 'inline' }}>
                                {t(runeword.name)}
                                {showPrevBadge(itemName) && (
                                  <small style={{ opacity: 0.6, marginLeft: 8, fontWeight: 400 }}>
                                    (Previously found)
                                  </small>
                                )}
                              </h3>
                            ...
                            secondary={
                              <RuneList>
                                {runeword.runes.map(runeName => {
                                  const rune = runes && runes[runeName] && runesMapping[runes[runeName] as RuneType];
                                  if (!rune) return null;
                                  return <Rune style={{ opacity: player[itemName] ? 1 : 0.5 }} key={runeName}>
                                    <RuneBg src={runeBgImg} />
